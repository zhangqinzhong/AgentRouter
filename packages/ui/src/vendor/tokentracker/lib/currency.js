...
export const SUPPORTED_CURRENCY_CODES = ["USD", "EUR", "GBP", "CNY", "JPY", "HKD"];
const CURRENCY_META = {
...
export function normalizeCurrency(value) {
    ...
    return SUPPORTED_CURRENCY_CODES.includes(upper)
        ? upper
        : CURRENCY_USD;
}
export function getCurrencySymbol(code) {
    const key = typeof code === "string" ? code.trim().toUpperCase() : CURRENCY_USD;
    return CURRENCY_META[key]?.symbol ?? "$";
}
export function getCurrencyLabelKey(code) {
...
export function getInitialExchangeRates() {
...
export function persistExchangeRates(state) {
...
export function getRateFor(rates, code) {
    if (code === CURRENCY_USD)
        return 1;
    const fromMap = rates?.[code];
    if (isValidRate(fromMap))
        return fromMap;
 ...
 */
export function applyCurrency(usdValue, code, rates) {
