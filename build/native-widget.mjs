...
export function buildNativeWidget(){
 ...
 function run(cmd,args){const r=spawnSync(cmd,args,{cwd:folder,stdio:'inherit'});if(r.status!==0){if(process.env.CI){console.warn('Widget build failed on CI; continuing without AgentRouterWidget');return false;}throw Error('Widget build failed: '+cmd);}return true;}
 if(!run('xcodegen',['generate','--spec',specFile,'--project',folder,'--project-root',folder]))return;
 const derived=path.join(folder,'.build');
 if(!run('xcodebuild',['-project','AgentRouterWidget.xcodeproj','-scheme','AgentRouterWidget','-configuration','Release','-derivedDataPath',derived,'-destination','generic/platform=macOS','ARCHS=arm64 x86_64','ONLY_ACTIVE_ARCH=NO','CODE_SIGNING_ALLOWED=NO','build']))return;
