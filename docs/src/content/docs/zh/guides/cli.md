...
```sh
git clone --branch v1.1.2 --depth 1 https://github.com/zhangqinzhong/AgentRouter.git
cd AgentRouter
npm ci
npm run build:assets
npm pack --workspace @zhangqinzhong/agentrouter --ignore-scripts
npm install -g ./zhangqinzhong-agentrouter-1.1.2.tgz
agentrouter --help
```
...
1. 添加供应商和至少一个模型。
2. 在 **API 密钥** 页面创建用于访问网关的 AgentRouter 客户端 Key。
3. 按需要设置默认模型、路由规则和回退。
4. 在侧边栏确认网关已经运行。
...
`start` 和 `ui` 会把进程 ID、URL 和私有服务 Token 写入 `service.json`。再次执行时，AgentRouter 会先验证对应进程和 RPC 身份：

- 服务有效时直接复用，不会再启动第二个后台进程。
- 新传入的 Host、Port 和 `--no-gateway` 不会重配已经运行的进程。
- 如果新命令要求网关运行，AgentRouter 会尝试在现有管理进程中启动网关。
- 状态文件失效或进程已经退出时，AgentRouter 会清理旧状态并启动新服务。
...
- Agent 自己的参数放到 `--` 后，避免与 AgentRouter 选项或入口名冲突。
...
大多数配置要求 AgentRouter 网关已经运行。Grok CLI、Kimi CLI 和 Pi 是例外：如果服务不存在，它们可以自动启动一个受管的临时共享服务，并在最后一个会话退出后关闭。
...
| `bin/` | AgentRouter 生成的 Agent 启动包装器。 |

不要在 AgentRouter 运行时直接编辑或复制活跃 SQLite 文件。优先使用 **Settings → Export data**；文件级备份前先停止 CLI 和桌面应用。
...
模型网关还需要单独创建 AgentRouter 客户端 Key。上游供应商凭据保存在本地数据目录，因此目录和备份都应按敏感数据保护。
...
管理服务可以在没有可用模型网关时运行。添加供应商和模型、创建 AgentRouter 客户端 Key，然后通过侧边栏的服务按钮启动或重启网关。使用 `agentrouter serve` 查看启动错误。

### 管理端口发生偏移

3458 已被占用，AgentRouter 使用了后续可用端口。以命令打印的 URL 为准；需要固定端口时，先停止冲突进程。

### 找不到 Agent 配置

确认配置已启用，并检查名称是否重复。AgentRouter 会按 ID、名称、忽略大小写的名称和清理后的名称匹配；多个结果时必须使用 ID。

### 提示启动器不存在

先打开一次 AgentRouter 或重新保存该 Agent 配置，让 AgentRouter 重新生成 `bin/` 下的启动包装器。
